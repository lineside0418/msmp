...
// Initialize configuration and update UI
async function initializeConfig() {
  ...
  if (serverIPElement) {
    serverIPElement.textContent = config.server.ip;
  }

  // Update instruction IP display
  const instructionIPElement = document.getElementById("instruction-ip");
  if (instructionIPElement) {
    instructionIPElement.textContent = config.server.ip;
  }

  // Update version display
  const versionElement = document.querySelector(".version-badge");
  if (versionElement) {
    versionElement.textContent = config.server.version;
  }

  // Update server name in title if needed
  ...
  if (
    ...
    heroTitle.textContent = config.server.name;
  }
}
...
async function updateServerStatus() {
  ...
  // Get server IP from config and update display
...
document.addEventListener("DOMContentLoaded", function () {
  const cards = document.querySelectorAll(
    ".feature-card, .staff-card, .rule-card"
  );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Initialize configuration first, then update server status
  ...
  try {
    ...
    if (data.members) {
      ...
      staffCards.forEach((card) => {
        ...
        const memberInfo = data.members.find(
      ...
      if (avatarElement && discordId.match(/^\d{17,19}$/)) {
        ...
        if (avatarElement) {
          ...
          if (staffInfo) {
            ...
            const avatarUrl =
              roleAvatars[staffInfo.role] ||
              `https://via.placeholder.com/150x150/059669/ffffff?text=Staff`;
